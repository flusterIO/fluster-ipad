...
- [ ] Handle conundrum parsing. Make sure front matter is being appended properly.
- [ ] Remove `get_conundrum_input` method completely to make sure everything is safe and implement the new approach everywhere.
- [ ] Resume work on website blog, get the model back up there ASAP.
- [ ] Review notes generated by AI while at the library.
- [ ] Create a `Conundrum` provider that functions similar to content-layer or the like, so you can build your notes and test in real time.
