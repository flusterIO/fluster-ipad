
> The following section was generated directly from code. That means the properties will always reflect the current state of the application, but on the other hand, they might not be formatted as neatly as the rest of the content.


#### superscript


**Boolean (optional)**

#### subscript


**Boolean (optional)**

#### idx


Automatically inserted by the compiler, this represents the index of the equation.

**Number** Automatically inserted by the compiler, this represents the index of the equation.


#### id


**String**

#### color


**Enum (optional):** One of the following:

- info
- error
- warn
- success
- primary
- important
- research
- highlight
- card

#### info


**Boolean (optional)**

#### error


**Boolean (optional)**

#### warn


**Boolean (optional)**

#### success


**Boolean (optional)**

#### primary


**Boolean (optional)**

#### important


**Boolean (optional)**

#### research


**Boolean (optional)**

#### highlight


**Boolean (optional)**

#### card


**Boolean (optional)**
