...
import { attachConundrumWasm } from "@conundrum/ts";
...
export const DictionaryPage = connector(
    ({ entries }: DictionaryPageProps): ReactNode => {
        const [wasmAttached, setWasmAttached] = useState(false)
        useEffect(() => {
            if (window.conundrum.compileConundrum) {
                setWasmAttached(true)
            } else {
                attachConundrumWasm(() => {
                    setWasmAttached(true)
                })
        ...
        if (!wasmAttached) {
            return (
                <div className="w-full h-fit min-h-screen flex flex-col justify-center items-center p-4">
                    <div className="text-2xl font-semibold">Attaching Conundrum Transpiler</div>
                </div>
            )
        }
        return (
