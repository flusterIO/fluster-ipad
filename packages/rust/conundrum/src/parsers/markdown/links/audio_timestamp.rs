...
pub struct AudioTimestampLink {
    pub target_id: DOMId,
}

impl ConundrumParser<AudioTimestampLink> for AudioTimestampLink {
    /// Matches the content inside the link, not the entire link.
    /// It matches `[](<THIS PART HERE ONLY)>`
    fn parse_input_string(input: &mut ConundrumInput) -> ConundrumModalResult<AudioTimestampLink> {
        ...
        Ok(AudioTimestampLink { target_id })
    }
