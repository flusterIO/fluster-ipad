...
pub struct LinkByDomId {
    pub target_id: DOMId,
}

impl ConundrumParser<LinkByDomId> for LinkByDomId {
    /// Matches the content inside the link, not the entire link.
    /// It matches `[](<THIS PART HERE ONLY)>`
    fn parse_input_string(input: &mut ConundrumInput) -> ConundrumModalResult<LinkByDomId> {
