...
impl ConundrumParser<MarkdownTableRow> for MarkdownTableRow {
    fn parse_input_string<'a>(input: &mut ConundrumInput<'a>) -> ConundrumModalResult<MarkdownTableRow> {
        let res = table_row_parser_wrapper(MarkdownTableCell::parse_input_string).parse_next(input)?;
...
mod tests {
    ...
    fn parses_basic_table_heading_row() {
        ...
        let res = MarkdownTableRow::parse_input_string.parse_next(&mut input)
                                                      .expect("Parses table row without throwing an error.");
