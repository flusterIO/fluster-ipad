...
pub fn space_or_tab(occurences: impl Into<Range> + Clone)
...
pub fn spaces_only(occurences: impl Into<Range> + Clone)
                   -> impl Fn(&mut ConundrumInput) -> ConundrumModalResult<String> {
    move |input: &mut ConundrumInput| take_while(occurences.clone(), |c| c == ' ').map(String::from).parse_next(input)
}
