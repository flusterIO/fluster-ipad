...
impl JsxPropertyParser for JsxObjectPropertyResult {
    fn parse_jsx_property(input: &mut ConundrumInput) -> ModalResult<JavascriptObjectKeyValuePair> {
        ...
        println!("Wrapped: {:#?}", wrapped_content);
        ...
        Ok(JavascriptObjectKeyValuePair { key,
                                          value: Box::new(ParsedJavascriptElement::Object(object_data)) })
    }
}
...
mod tests {
    ...
    fn parses_jsx_object_prop() {
        ...
        let res = JsxObjectPropertyResult::parse_jsx_property(&mut test_data).expect("Parses object without throwing an error.");
        ...
        assert!(string_value.is_some_and(|s| match s {
                                ParsedJavascriptElement::String(j) => j.value == "my value",
                                _ => false,
                            }),
