...
pub struct KatexFontAssetFrakturRegular(WebGlueCodeGeneralFiles);

impl KatexFontAssetFrakturRegular {
    fn get_font_data(&self) -> String {
        if let Some(data) = JS_GLUE_CODE_MAP::get_bytes_by_key(AnyGlueCodeKey::General(self.0.clone())) {
            ...
            eprintln!("The KatexFontAssetFrakturRegular font could not be loaded");
            String::from("")
        }
    }
}

impl Default for KatexFontAssetFrakturRegular {
    fn default() -> Self {
        Self(WebGlueCodeGeneralFiles::Katex_fraktur_regular)
    }
}

impl GlueCssAsset for KatexFontAssetFrakturRegular {
