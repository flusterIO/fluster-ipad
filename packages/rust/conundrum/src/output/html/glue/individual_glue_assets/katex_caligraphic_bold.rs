...
pub struct KatexFontAssetCaligraphicBold(WebGlueCodeGeneralFiles);

impl KatexFontAssetCaligraphicBold {
    fn get_font_data(&self) -> String {
        if let Some(data) = JS_GLUE_CODE_MAP::get_bytes_by_key(AnyGlueCodeKey::General(self.0.clone())) {
            ...
            eprintln!("The KatexFontAssetCaligraphicBold font could not be loaded");
            String::from("")
        }
    }
}

impl Default for KatexFontAssetCaligraphicBold {
    fn default() -> Self {
        Self(WebGlueCodeGeneralFiles::Katex_caligraphic_bold)
    }
}

impl GlueCssAsset for KatexFontAssetCaligraphicBold {
