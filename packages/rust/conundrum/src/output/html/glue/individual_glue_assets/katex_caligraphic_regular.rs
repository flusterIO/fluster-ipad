...
pub struct KatexFontAssetCaligraphicRegular(WebGlueCodeGeneralFiles);

impl KatexFontAssetCaligraphicRegular {
    fn get_font_data(&self) -> String {
        if let Some(data) = WEB_GLUE_CODE_MAP::get_font_bytes_by_key(AnyComponentKey::General(self.0.clone())) {
            ...
            eprintln!("The KatexFontAssetCaligraphicRegular font could not be loaded");
            String::from("")
        }
    }
}

impl Default for KatexFontAssetCaligraphicRegular {
    fn default() -> Self {
        Self(WebGlueCodeGeneralFiles::Katex_caligraphic_regular)
    }
}

impl GlueCssAsset for KatexFontAssetCaligraphicRegular {
