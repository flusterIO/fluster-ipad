...
pub struct KatexFontAssetSansserifBold(WebGlueCodeGeneralFiles);

impl KatexFontAssetSansserifBold {
    fn get_font_data(&self) -> String {
        if let Some(data) = WEB_GLUE_CODE_MAP::get_font_bytes_by_key(AnyComponentKey::General(self.0.clone())) {
            ...
            eprintln!("The KatexFontAssetSansserifBold font could not be loaded");
            String::from("")
        }
    }
}

impl Default for KatexFontAssetSansserifBold {
    fn default() -> Self {
        Self(WebGlueCodeGeneralFiles::Katex_sansserif_bold)
    }
}

impl GlueCssAsset for KatexFontAssetSansserifBold {
