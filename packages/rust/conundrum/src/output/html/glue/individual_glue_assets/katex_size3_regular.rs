...
pub struct KatexFontAssetSize3Regular(WebGlueCodeGeneralFiles);

impl KatexFontAssetSize3Regular {
    fn get_font_data(&self) -> String {
        if let Some(data) = JS_GLUE_CODE_MAP::get_bytes_by_key(AnyGlueCodeKey::General(self.0.clone())) {
            ...
            eprintln!("The KatexFontAssetSize3Regular font could not be loaded");
            String::from("")
        }
    }
}

impl Default for KatexFontAssetSize3Regular {
    fn default() -> Self {
        Self(WebGlueCodeGeneralFiles::Katex_size3_regular)
    }
}

impl GlueCssAsset for KatexFontAssetSize3Regular {
