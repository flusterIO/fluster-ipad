...
pub struct KatexFontAssetMathBolditalic(WebGlueCodeGeneralFiles);

impl KatexFontAssetMathBolditalic {
    fn get_font_data(&self) -> String {
        if let Some(data) = WEB_GLUE_CODE_MAP::get_font_bytes_by_key(AnyComponentKey::General(self.0.clone())) {
            ...
            eprintln!("The KatexFontAssetMathBolditalic font could not be loaded");
            String::from("")
        }
    }
}

impl Default for KatexFontAssetMathBolditalic {
    fn default() -> Self {
        Self(WebGlueCodeGeneralFiles::Katex_math_bolditalic)
    }
}

impl GlueCssAsset for KatexFontAssetMathBolditalic {
