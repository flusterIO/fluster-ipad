...
pub struct KatexFontAssetFrakturBold(WebGlueCodeGeneralFiles);

impl KatexFontAssetFrakturBold {
    fn get_font_data(&self) -> String {
        if let Some(data) = JS_GLUE_CODE_MAP::get_bytes_by_key(AnyGlueCodeKey::General(self.0.clone())) {
            ...
            eprintln!("The KatexFontAssetFrakturBold font could not be loaded");
            String::from("")
        }
    }
}

impl Default for KatexFontAssetFrakturBold {
    fn default() -> Self {
        Self(WebGlueCodeGeneralFiles::Katex_fraktur_bold)
    }
}

impl GlueCssAsset for KatexFontAssetFrakturBold {
