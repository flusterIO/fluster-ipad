...
pub struct KatexFontAssetTypewriterRegular(WebGlueCodeGeneralFiles);

impl KatexFontAssetTypewriterRegular {
    fn get_font_data(&self) -> String {
        if let Some(data) = WEB_GLUE_CODE_MAP::get_font_bytes_by_key(AnyComponentKey::General(self.0.clone())) {
            ...
            eprintln!("The KatexFontAssetTypewriterRegular font could not be loaded");
            String::from("")
        }
    }
}

impl Default for KatexFontAssetTypewriterRegular {
    fn default() -> Self {
        Self(WebGlueCodeGeneralFiles::Katex_typewriter_regular)
    }
}

impl GlueCssAsset for KatexFontAssetTypewriterRegular {
