...
pub struct KatexFontAssetSize1Regular(WebGlueCodeGeneralFiles);
...
impl KatexFontAssetSize1Regular {
    fn get_font_data(&self) -> String {
        if let Some(data) = JS_GLUE_CODE_MAP::get_bytes_by_key(AnyGlueCodeKey::General(self.0.clone())) {
            ...
            eprintln!("The KatexFontAssetSize1Regular font could not be loaded");
            String::from("")
        }
    }
}

impl Default for KatexFontAssetSize1Regular {
    fn default() -> Self {
        Self(WebGlueCodeGeneralFiles::Katex_size1_regular)
    }
}

impl GlueCssAsset for KatexFontAssetSize1Regular {
