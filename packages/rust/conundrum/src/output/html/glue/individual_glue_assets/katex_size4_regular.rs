...
pub struct KatexFontAssetSize4Regular(WebGlueCodeGeneralFiles);

impl KatexFontAssetSize4Regular {
    fn get_font_data(&self) -> String {
        if let Some(data) = JS_GLUE_CODE_MAP::get_bytes_by_key(AnyGlueCodeKey::General(self.0.clone())) {
            ...
            eprintln!("The KatexFontAssetSize4Regular font could not be loaded");
            String::from("")
        }
    }
}

impl Default for KatexFontAssetSize4Regular {
    fn default() -> Self {
        Self(WebGlueCodeGeneralFiles::Katex_size4_regular)
    }
}

impl GlueCssAsset for KatexFontAssetSize4Regular {
