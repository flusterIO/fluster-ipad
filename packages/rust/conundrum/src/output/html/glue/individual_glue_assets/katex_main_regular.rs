...
pub struct KatexFontAssetMainRegular(WebGlueCodeGeneralFiles);

impl KatexFontAssetMainRegular {
    fn get_font_data(&self) -> String {
        if let Some(data) = WEB_GLUE_CODE_MAP::get_font_bytes_by_key(AnyComponentKey::General(self.0.clone())) {
            ...
            eprintln!("The KatexFontAssetMainRegular font could not be loaded");
            String::from("")
        }
    }
}

impl Default for KatexFontAssetMainRegular {
    fn default() -> Self {
        Self(WebGlueCodeGeneralFiles::Katex_main_regular)
    }
}

impl GlueCssAsset for KatexFontAssetMainRegular {
