...
pub struct KatexFontAssetMainItalic(WebGlueCodeGeneralFiles);

impl KatexFontAssetMainItalic {
    fn get_font_data(&self) -> String {
        if let Some(data) = JS_GLUE_CODE_MAP::get_bytes_by_key(AnyGlueCodeKey::General(self.0.clone())) {
            ...
            eprintln!("The KatexFontAssetMainItalic font could not be loaded");
            String::from("")
        }
    }
}

impl Default for KatexFontAssetMainItalic {
    fn default() -> Self {
        Self(WebGlueCodeGeneralFiles::Katex_main_italic)
    }
}

impl GlueCssAsset for KatexFontAssetMainItalic {
