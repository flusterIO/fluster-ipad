...
#if canImport(fluster_desktop_fsFFI)
import fluster_desktop_fsFFI
#endif
...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_fluster_desktop_fs_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_fluster_desktop_fs_rustbuffer_free(self, $0) }
    }
}
...
private func makeRustCall<T, E: Swift.Error>(
    ...
    uniffiEnsureFlusterDesktopFsInitialized()
...
open class Greeter: GreeterProtocol, @unchecked Sendable {
    ...
    public func uniffiClonePointer() -> UnsafeMutableRawPointer {
        return try! rustCall { uniffi_fluster_desktop_fs_fn_clone_greeter(self.pointer, $0) }
    }
public convenience init(name: String) {
    let pointer =
        try! rustCall() {
    uniffi_fluster_desktop_fs_fn_constructor_greeter_new(
    ...
    deinit {
        ...
        try! rustCall { uniffi_fluster_desktop_fs_fn_free_greeter(pointer, $0) }
    }
...
open func greet() -> String  {
    ...
    uniffi_fluster_desktop_fs_fn_method_greeter_greet(self.uniffiClonePointer(),$0
    )
})
}
...
public func add(a: UInt64, b: UInt64) -> UInt64  {
    ...
    uniffi_fluster_desktop_fs_fn_func_add(
...
private let initializationResult: InitializationResult = {
    ...
    let scaffolding_contract_version = ffi_fluster_desktop_fs_uniffi_contract_version()
    if bindings_contract_version != scaffolding_contract_version {
        return InitializationResult.contractVersionMismatch
    }
    if (uniffi_fluster_desktop_fs_checksum_func_add() != 36805) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_fluster_desktop_fs_checksum_method_greeter_greet() != 31201) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_fluster_desktop_fs_checksum_constructor_greeter_new() != 5432) {
        return InitializationResult.apiChecksumMismatch
    }
...
public func uniffiEnsureFlusterDesktopFsInitialized() {
