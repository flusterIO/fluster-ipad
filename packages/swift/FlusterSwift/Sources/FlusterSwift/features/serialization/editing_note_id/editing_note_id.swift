...
public class EditingNoteId: Codable {
    ...
    public init(value: String) {
        ...
        if value.starts(with: EditingNoteId.editingNoteIdProtocolLeader) {
            ...
            self.id = self.value.replacingOccurrences(of: EditingNoteId.editingNoteIdProtocolLeader, with: "")
        } else {
    ...
    public func getEditingNoteId(context: ModelContext) async -> NoteModel? {
        let _id = self.id
        let fetchDescriptor = self.isUserDefined ? FetchDescriptor<NoteModel>(
